 ...
 * 充值卡余额明细列表适配器
 */
public class Z2_Adapter extends BaseAdapter {
    private Context c;
    public ArrayList<pd_List_data> List;
    private LayoutInflater mInflater;


    public Z2_Adapter(Context c, ArrayList<pd_List_data> dataList) {
    ...
    public View getView(final int position, View cellView, ViewGroup parent) {
        ...
        tv_explain.setText(List.get(position).lg_desc);
        tv_time.setText(List.get(position).lg_add_time);
        if (Double.parseDouble(List.get(position).lg_av_amount) > 0) {
            tv_tagging.setText("收入");
            tv_amount.setTextColor(Color.parseColor("#f25252"));
            tv_amount.setText("+" + List.get(position).lg_av_amount);
        } else {
